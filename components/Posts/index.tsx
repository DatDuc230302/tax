    ...
    return (
        <div className="flex flex-col mt-4">
            <div className=" flex flex-col gap-2 items-center lg:flex-row justify-between">
                <h2 className="text-[30px]">Văn bản</h2>
                <div className="flex gap-2">
                    {listPosts.map((item: items, index: number) => (
                        <span
                            onClick={() => handleChange(index)}
                            className={`${
                                active === index ? 'text-red-500 border-red-500' : 'text-black'
                            } flex font-bold ease-linear duration-200 cursor-pointer text-[14px] h-full items-center pb-2 border-b-[2px] border-transparent`}
                            key={index}
                        >
                            {item.title}
                        </span>
                ...
                {arr.map((item: number) => (
                    <div key={item} className="flex-col flex gap-1 lg:gap-0 lg:flex-row lg:justify-between py-4">
                        <div className="flex items-center h-full gap-6">
                            ...
                            <span className="pr-[10px] flex font-bold cursor-pointer hover:text-red-500 duration-100 ease-linear">
